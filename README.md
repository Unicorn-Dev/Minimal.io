...
1. На счет установщика, мы правдо пытались, но не вышло( pyinstaller плохо справляется с системой файлов и у нас с ним не вышло, py2app смравился в режиме дебага (python3 setup.py py2app -A) и создал рабочее приложение, но при финальном шаге (python3 setup.py py2app) он падает, и мы не можем понять почему...
